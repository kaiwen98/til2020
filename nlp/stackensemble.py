...
def classifier(model_name, emb_mean, emb_std, embeddings_index):
    ...
    d_train = pd.read_csv('./input/TIL_NLP_train1_dataset.csv')
    d_test = pd.read_csv('./input/TIL_NLP_unseen_dataset.csv')
    print('running classifier')

    tokenizer = text.Tokenizer(num_words=max_features)
    x_train, x_test, y_train, y_test, tokenizer = datagen(d_train, d_test)
    ...
    #supermodel.load_weights('final_ensemble_complete.h5')
...
if __name__ == "__main__":
    ...
    model_name = "final_ensemble_complete.h5"
