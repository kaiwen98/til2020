...
from keras.layers import Input, Dense, Embedding, MaxPooling1D, Conv1D, SpatialDropout1D, Activation
...
def classifier(model, emb_mean, emb_std, embeddings_index):
    ...
    y_val = yval.tolist()
    accuracy = sum([y_pred[i] == y_val[i] for i in range(len(y_pred))])/len(y_pred) * 100
